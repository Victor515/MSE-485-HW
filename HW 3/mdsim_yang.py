...
def compute_energy(pset,box_length):
    ...
    temperature = tot_kinetic / (1.5*pset.size()) # the temperature of the system in the reduced units
...
def rhok(kvec, pset):
    ...
    for iatom in range(pset.size()):
        kr = np.dot(kvec, pset.pos(iatom)) # dot product of k vector and atom position
...
def Sk(kvecs, pset):
    ...
    for ivec in kvecs:
        ...
        sk_list[index] += abs(rho_k)**2/pset.size()
        index += 1
    return sk_list
# end def Sk
...
if __name__ == '__main__':
    ...
    maxk = 5             # defines the maximum of n_x, n_y and n_z in the k vector
    S_size = (maxk+1)**3-1   # the size of S list, or the number of legal k vectors, eliminating vector [0,0,0]
    S = np.zeros(S_size) # structure factor
    kvecs = legal_kvecs(maxk,pset,box_length) # legal k vectors considered in Sk calculation
    ...
    for istep in range(nsteps):
        ...
        # calculate g(r) and S(k) after equilibrium
        if istep > step_equil:
            # calculate g(r)
    ...
    for ihis in range(nhis):
        dV = (4/3)*math.pi * ((ihis+1)**3-ihis**3)*dr**3 # the volume of the shell considered
        num_ideal = num_atoms*dV/V  # number of ideal gas atoms corresponding in dV
